...
## 4.5 Sandboxテストの落とし穴（2026-06-07の検証で得た知見）

- **テスト購入はApple ID（テスター）単位で所有される**。買い切り商品は同じテスターで
  再テストすると自動復元されて「全部購入済み」になる — これは正常な挙動
- **「購入履歴を消去」では綺麗な状態に戻らないことが多い**:
  - Apple側の反映遅延 / 端末のStoreKit 2キャッシュが旧所有情報を再配信 /
    RevenueCat側に旧購入が残っていて新ユーザーに引き継がれる（transfer）
- **「未購入ユーザー」を再現したいときは、必ず新しいSandboxテスターを作る**:
  1. ASCでテスター新規作成（地域: 日本）
  2. 実機の 設定 > App Store > サンドボックスアカウント でサインアウト→新テスターでサインイン
  3. **端末を再起動**（SK2キャッシュ対策）→ アプリ削除 → 再インストール
- アプリごとにテスターを分けると状態管理が楽（Morphica用と共用しない）
- RCダッシュボードの**カスタマーリストは定期バッチ更新**（最大24時間遅延）。
  リアルタイム確認は Overview の Recent sandbox transactions か、カスタマーIDの直接検索で
