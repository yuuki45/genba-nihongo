...
#### アプリ内課金 フェーズ1: JLPT N3/N2対策パック ✅（2026-06-07追加）
- ✅ 対策クイズ140問を制作（quizzes.json data_version: 3、id 101-240、pack_id: jlpt_n3n2）
  - N3対策70問 / N2対策70問（各: 文法25 + 語彙25 + 漢字読み20、現場文脈の出題）
  - 漢字読み問題の語は漢字カード100語と重複しない（テストで担保）
- ✅ クイズのレベル対応（randomQuizzesProviderに解錠フィルタ、QuizScreenにjlptLevelパラメータ）
- ✅ ホームに「N2クイズに挑戦」カード（未購入時はロック表示→ストア誘導）
- ✅ 新規問題のローマ字行の重複表示を修正（マップにない選択肢は非表示）
- ✅ クイズデータ整合性テスト9件追加（パック構成・4択構造・解説と正答の一致、計77テスト）
- ⏳ 残: App Store Connect商品登録、StoreKit Testing/Sandboxでの購入検証

#### 無料N3クイズの品質監査・全面改修 ✅（2026-06-07追加）
詳細: `docs/free-quiz-audit.md`
- ✅ 全100問を精査し、正答キー誤り2問・非文2問・複数正解問題15問を修正
- ✅ 語彙32問を定義型から有料版と同じ文中空欄ドリル形式に全面書き直し
- ✅ 無料/有料の語彙正答の重複を排除（テストで恒久担保）
- ✅ 全問題の解説を「正解は「X」。」形式に統一し、解説と正答の一致を全問テストで検証
- ✅ quizzes.json data_version: 4（既存ユーザーにも修正版が自動配信）

#### アプリ内課金 フェーズ2: 介護フレーズパック ✅（2026-06-07追加）
- ✅ 介護フレーズ100件を制作（phrases.json data_version: 4、カテゴリ6・pack_id: kaigo）
  - 声かけ・食事・入浴・排泄・移乗・体調・整容・レク・申し送りの9場面
- ✅ カテゴリタブのロック表示（🔒）と未購入カテゴリの5件プレビュー + 解錠バナー
- ✅ 検索・今日の3フレーズに解錠フィルタを適用（未購入フレーズは非表示）
- ✅ カテゴリロック判定の純粋関数 + テスト（計87テスト）
- ⏳ 残: App Store Connect商品登録（com.genba.nihongo.pack.kaigo）

#### フレーズ一覧のシーン分割 ✅（2026-06-07追加）
- ✅ シーン選択（ハブ）画面を新設（PhraseSceneScreen）
  - 🏭 工場・現場（安全・作業指示・緊急）/ 💬 日常・あいさつ / 🤲 介護（ロックバッジ付き）
  - 各シーンの収録フレーズ数を表示、検索導線をハブに配置
- ✅ フレーズ一覧画面をシーン単位に変更（PhraseScene定数でカテゴリをグルーピング）
  - シーン内カテゴリのみのタブ表示（カテゴリ1つのシーンはタブ非表示）
  - シーン遷移時にフィルタ状態をリセット
- ✅ シーン×カテゴリのマッピング整合性テスト追加（計88テスト）
- 新職種パック追加時はPhraseScene.allに1エントリ追加するだけで同UIが機能
